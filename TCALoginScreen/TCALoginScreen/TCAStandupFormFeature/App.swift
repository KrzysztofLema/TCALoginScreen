...
struct AppView: View {
    ...
    var body: some View {
        NavigationStackStore(self.store.scope(state: \.path, action: { .path($0)}), root: {
            StandupsListView(store: self.store.scope(
                state: \.standupsList,
            ...
            switch state {
            case .detail:
                CaseLet(/AppFeature.Path.State.detail, action: AppFeature.Path.Action.detail, then: StandupDetailView.init(store:))
            }
        })
    }
}
#Preview {
    AppView(store: Store(initialState: AppFeature.State(standupsList: StandupsListFeature.State(standups: [.mock])), reducer: {
        AppFeature()
            ._printChanges()
    }))
}
